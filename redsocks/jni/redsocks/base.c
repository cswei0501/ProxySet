...
static int base_init()
{
	...
	if (!instance.configured) {
		fprintf(stderr,"[%d]there is no configured instance of `base`, check config file\n",getpid());
		return -1;
	}
	...
	fprintf(stderr,"[%d]log open\n",getpid());
	if (instance.daemon) {
		if (setsid() < 0) {
			fprintf(stderr,"[%d]setsid()",getpid());
			goto fail;
		}
		fprintf(stderr,"[%d]setsid()\n",getpid());
		int fds[] = { STDIN_FILENO, STDOUT_FILENO, STDERR_FILENO };
		int *pfd;
		fprintf(stderr,"STDIN_FILENO->%d\tSTDOUT_FILENO->%d\tSTDERR_FILENO->%d\n",STDIN_FILENO, STDOUT_FILENO, STDERR_FILENO);
		FOREACH(pfd, fds)
		{
			fprintf(stderr,"[%d]FOREACH fds:%d\n",getpid(),*pfd);
			//if (dup2(devnull, *pfd) < 0) {
			//	fprintf(stderr,"[%d]dup2(devnull, %i)",getpid(), *pfd);
			//	goto fail;
			//}
			fprintf(stderr,"[%d]FOREACH end\n",getpid());
		}
		fprintf(stderr,"[%d]close devnull\n",getpid());	
		close(devnull);
	}
	fprintf(stderr,"[%d]base init success\n",getpid());
	return 0;
fail:
	fprintf(stderr,"[%d]base init fail\n",getpid());
	if (devnull != -1)
		close(devnull);
