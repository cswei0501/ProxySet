...
static void terminate(int sig, short what, void *_arg)
{
	if (event_loopbreak() != 0)
		fprintf(stderr, "event_loopbreak");
}

int main(int argc, char **argv)
{	
	fprintf(stderr,"[%d]redsocks start....\n\n",getpid());
	int error;
	app_subsys **ss;
	...
	fprintf(stderr,"[%d]conf file path -->%s\n",getpid(),confname);
	...
	if (!parser) {
		fprintf(stderr,"[%d]Not enough memory for parser",getpid());
		return EXIT_FAILURE;
	}
	fprintf(stderr,"[%d]parser_start conf file\n",getpid());
	...
	if (conftest)
		return EXIT_SUCCESS;
	fprintf(stderr,"[%d]event_init....\n",getpid());
	struct event_base* evbase = event_init();
	memset(terminators, 0, sizeof(terminators));
	fprintf(stderr,"[%d]event_init ed\n",getpid());
	FOREACH(ss, subsystems) {
		fprintf(stderr,"[%d]FOREACH start\n",getpid());
		if ((*ss)->init) {
			error = (*ss)->init(evbase);
			if (error)
			{
				fprintf(stderr,"[%d]goto shutdown...\n",getpid());
				goto shutdown;
			}
			fprintf(stderr,"[%d](*ss)->init \n",getpid());	
		}
		fprintf(stderr,"[%d]FOREACH\n",getpid());
	}
	fprintf(stderr,"[%d]pid file path -->%s\n",getpid(),pidfile);
	if (pidfile) {
		f = fopen(pidfile, "w");
		if (!f) {
	...
	fprintf(stderr,"[%d]redsocks started\n\n",getpid());

	event_dispatch();

	fprintf(stderr, "[%d]redsocks goes down",getpid());
